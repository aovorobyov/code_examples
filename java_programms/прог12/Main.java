...
class ExpressTrip extends Trip {
    public double LengthCoefficient;
    private String StartPoint, FinalPoint; //переменные для назв. нач. и кон. пунктов
    private int RouteNum; //переменные с номером маршрута
    private double TravelTime, RouteLen, TicketPrice; //его длительностью, длиной, ценой билета.
    public ExpressTrip(String SP, String FP, int RN, double TT, double RL, double TP, double LC) {
        super(SP, FP, RN, TT, RL, TP);
        this.StartPoint = SP;
        this.FinalPoint = FP;
        this.RouteNum = RN;
        this.TravelTime = TT;
        this.RouteLen = RL;
        this.TicketPrice = TP;
        this.LengthCoefficient = LC;
    }
    public ExpressTrip() { //конструктор2
        this.StartPoint = "Moscow";
        this.FinalPoint = "Kazan";
        this.RouteNum = 1015;
        this.TravelTime = 12.15;
        this.RouteLen = 820;
        this.TicketPrice = 1703;
        this.LengthCoefficient = 0.98;
    }

    static void ExpressRouteLength (ExpressTrip e1, ExpressTrip e2, ExpressTrip e3,
                                    ExpressTrip e4, ExpressTrip e5, ExpressTrip e6) {

        double[] len; //массив с длиной маршрута
        len = new double[n];
        len[0] = e1.RouteLen; len[1] = e2.RouteLen; len[2] = e3.RouteLen;
        len[3] = e4.RouteLen; len[4] = e5.RouteLen; len[5] = e6.RouteLen;

        double[] coef; //массив с коэффицентами
        coef = new double[n];
        coef[0] = e1.LengthCoefficient; coef[1] = e2.LengthCoefficient; coef[2] = e3.LengthCoefficient;
        coef[3] = e4.LengthCoefficient; coef[4] = e5.LengthCoefficient; coef[5] = e6.LengthCoefficient;

        String[] destination; //массив с пунктами назначения
        destination = new String[n]; // заполняем значения
        destination[0] = e1.FinalPoint; destination[1] = e2.FinalPoint; destination[2] = e3.FinalPoint;
        destination[3] = e4.FinalPoint; destination[4] = e5.FinalPoint; destination[5] = e6.FinalPoint;

        for (int l = 0; l < n; l++) {
            double explen = len[l] * coef[l];
            System.out.println("Express trip to " + destination[l] + " is: " + Math.ceil(explen) + " km.");
        }
    }

    static void NewPrice(ExpressTrip e1, ExpressTrip e2, ExpressTrip e3,
                         ExpressTrip e4, ExpressTrip e5, ExpressTrip e6) {
        double[] price; //массив с ценами на поездки
        price = new double[n]; //заполняем значения
        price[0] = e1.TicketPrice; price[1] = e2.TicketPrice; price[2] = e3.TicketPrice;
        price[3] = e4.TicketPrice; price[4] = e5.TicketPrice; price[5] = e6.TicketPrice;
        ...
        for (int l = 0; l < n; l++) {
            double newpr = price[l] * (1 + (1 - coef[l]));
            System.out.println("A bus ticket to " + destination[l] + " by Express route costs " + Math.ceil(newpr) + " Rub.");
        }
    }
}
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //вызов методов(12)
        Trip.TimeDifference(trip1, trip2); //разница во времени между поездками
        Trip.ShortestTrip(trip1, trip2, trip3, trip4, trip5, trip6);
        Trip.LongestTrip(trip1, trip2, trip3, trip4, trip5, trip6);
        Trip.InfoAboutRoutes(trip1, trip2, trip3, trip4, trip5, trip6);
        Trip.AverageSpeed(trip1, trip2, trip3, trip4, trip5, trip6);
        Trip.LongestRoute(trip1, trip2, trip3, trip4, trip5, trip6);
        Trip.ShortestRoute(trip1, trip2, trip3, trip4, trip5, trip6);
        Trip.GetPrice(trip1, trip2, trip3, trip4, trip5, trip6);
        Trip.StudentSale30(trip1, trip2, trip3, trip4, trip5, trip6);
        Trip.ChildTariff50(trip1, trip2, trip3, trip4, trip5, trip6);
        Trip.CheapestTrip(trip1, trip2, trip3, trip4, trip5, trip6);
        Trip.MostExpensiveTrip(trip1, trip2, trip3, trip4, trip5, trip6);

        System.out.println("________________________________________________________");

        //данные для нового класса
        ExpressTrip etrip1 = new ExpressTrip("Moscow", "Kiev", 560, 10.9, 860, 3405, 0.81);
        ExpressTrip etrip2 = new ExpressTrip("Moscow", "Minsk", 915, 7.9, 680, 1589, 0.76);
        ExpressTrip etrip3 = new ExpressTrip("Moscow", "Sevastopol", 440, 20.5, 1900, 3105, 0.9);
        ExpressTrip etrip4 = new ExpressTrip("Moscow", "Saint-Petersburg", 685, 7.5, 640, 1163, 0.85);
        ExpressTrip etrip5 = new ExpressTrip("Moscow", "Arkhangelsk", 590, 16.1, 1200, 2400, 0.78);           ExpressTrip etrip6 = new ExpressTrip();

        //методы нового класса
        ExpressTrip.ExpressRouteLength(etrip1, etrip2, etrip3, etrip4, etrip5, etrip6);
        ExpressTrip.NewPrice(etrip1, etrip2, etrip3, etrip4, etrip5, etrip6);
    }
}
